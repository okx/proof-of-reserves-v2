...
pub fn recursive_prove_subproofs<const RECURSION_FACTOR: usize>(
    ...
    for level in 0..recursive_levels {
        ...
        let chunks: Vec<Vec<Vec<ProofWithPublicInputs<F, C, D>>>> = subproof_batches
            .into_iter()
            .chunks(parallism)
            .into_iter()
            .map(|chunk| chunk.collect())
        ...
        chunks.into_iter().for_each(|chunk: Vec<Vec<ProofWithPublicInputs<F, C, D>>>| {
            let proofs: Vec<ProofWithPublicInputs<F, C, D>> = chunk
                .into_par_iter()
                .map(|subproofs| {
                    let sub_proofs: [ProofWithPublicInputs<F, C, D>; RECURSION_FACTOR] = subproofs
                        .try_into()
