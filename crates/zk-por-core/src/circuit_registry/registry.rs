...
use crate::{
    ...
    merkle_sum_prover::{
        circuits::{merkle_sum_circuit::build_merkle_sum_tree_circuit, account_circuit::AccountTargets},
...
pub struct CircuitRegistry {
	...
	recursive_circuits_and_empty_proofs : Vec<((CircuitData<F, C, D>, RecursiveTargets<RECURSIVE_FACTOR>), ProofWithPublicInputs<F, C, D>)>,
}

fn gen_empty_accounts(
    num_accounts: usize,
    num_assets: usize,
) -> (Vec<Account>, u32, u32) {
...
impl CircuitRegistry {
	pub fn init() -> Self {
		...
		let (batch_circuit_data, account_targets) = build_merkle_sum_tree_circuit(BATCH_SIZE, ASSET_NUM);
		tracing::info!("build merkle sum tree circuit with batch size {} in : {:?}", BATCH_SIZE, start.elapsed());

		let accounts = gen_empty_accounts(BATCH_SIZE, ASSET_NUM).0;

		let start = std::time::Instant::now();
		let prover = MerkleSumTreeProver { accounts: accounts};
        let empty_batch_proof = prover.prove_with_circuit(&batch_circuit_data, account_targets.clone()).unwrap();
		tracing::info!("prove merkle sum tree with batch size {} in : {:?}", BATCH_SIZE, start.elapsed());
		...
		loop {
			let start = std::time::Instant::now();
			let (recursive_circuit, recursive_account_targets) = build_recursive_n_circuit::<C, RECURSIVE_FACTOR>(&batch_circuit_data.common, &batch_circuit_data.verifier_only);
			...
			tracing::info!("prove recursive subproofs at level {} in : {:?}", level, start.elapsed());
			if recursive_circuit.verifier_only.circuit_digest == last_circuit_data.verifier_only.circuit_digest {
				break;	
			}

			recursive_circuit_and_empty_proofs.push(((recursive_circuit, recursive_account_targets), recursive_proof));	

			level += 1;
			last_circuit_data = &recursive_circuit_and_empty_proofs.last().unwrap().0.0;
		}

		tracing::info!("finish init circuit registry with {} recursive levels in {:?}", level, init_start.elapsed());
	...
	pub fn get_recursive_circuit(&self, level: usize) -> (&CircuitData<F, C, D>, RecursiveTargets<RECURSIVE_FACTOR>) {
	...
	pub fn get_empty_recursive_circuit_proof(&self, level: usize) -> ProofWithPublicInputs<F, C, D> {
		let mut circuit_and_empty_proof = self.recursive_circuits_and_empty_proofs.last().unwrap();
		if level < self.recursive_circuits_and_empty_proofs.len() {
			circuit_and_empty_proof = self.recursive_circuits_and_empty_proofs.get(level).unwrap();
		}
